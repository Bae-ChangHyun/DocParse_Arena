...
class ClaudeOcrProvider(OcrProvider):
    ...
    def _build_messages(self, b64_image: str, mime_type: str, user_prompt: str) -> list:
        return [
            {
                "role": "user",
                "content": [
    ...
    async def process_image(
        ...
        try:
            ...
            if prompt:
                api_kwargs["system"] = prompt
            response = await self.client.messages.create(
                model=self.model_id,
                messages=self._build_messages(b64_image, mime_type, user_prompt),
                **api_kwargs,
            )
